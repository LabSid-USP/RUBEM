...
Mandatory maximum float value [dimensionless quantity] that characterizes plant canopies. It is defined as the one-sided green leaf area per unit ground surface area. When ``lai_max_from_table`` is ``true`` (see below) the value no longer enters the LAI, which takes the maximum of each land-use class from the ``lai_max`` table; ``lai_max`` remains a mandatory constant and is still validated against its range.
...
The table is an opt-in extension of the published formulation, in which :math:`LAI_{MAX}` is a single value for the whole catchment. It is read only when the constant ``lai_max_from_table`` is ``true``; the table is then mandatory and the ``lai_max`` constant no longer enters the LAI (it remains mandatory and validated). With ``lai_max_from_table`` absent or ``false`` (the default) the constant is used; a table that is given must exist but is otherwise ignored, which is reported. When the table is read, its values must be positive and at most 12, and it must list, one per row, the land-use classes of the area fraction tables.
...
Default ``False`` boolean, the raster data generated by the model will be exported in TIFF/GeoTIFF map format. See the `related documentation <https://gdal.org/drivers/raster/gtiff.html>`__ for more information.
...
Optional finite number written as the no-data value of the TIFF/GeoTIFF raster series (declared on the band and stored in the cells the model leaves missing). Default ``-9999``.
