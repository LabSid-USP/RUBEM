...
def series_extremes(
...
) -> tuple[np.ndarray, np.ndarray, np.ndarray, RasterData, np.dtype]:
    ...
    :return: ``(minimum, maximum, valid, reference, dtype)`` where ``valid``
        marks the cells with at least one value, ``reference`` carries the
        geometry (and the projection of ``georeference`` when given) and
        ``dtype`` is the common type of the series' bands, which every
        extreme fits.
    """
    ...
    for file in files:
        ...
        dtype = np.promote_types(dtype, data.array.dtype)
        mask = data.mask()
    ...
    return minimum, maximum, valid, reference, dtype
...
def minmax(
    ...
    The outputs take the common floating type of the series' bands (Float32
    for Float32 or narrower integer sources, Float64 when any source is
    Float64 or a wide integer), widened if ``nodata`` does not fit it; a
    valid extreme is never cast to a type that cannot represent it.
    ...
    minimum, maximum, valid, reference, dtype = series_extremes(inputs, georeference)
    dtype = dtype_for_nodata(np.promote_types(dtype, np.float32), nodata)
    minimum = np.where(valid, minimum, nodata).astype(dtype)
    maximum = np.where(valid, maximum, nodata).astype(dtype)
